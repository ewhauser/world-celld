...
| v0.5.0 service-binding RPC                                                                                   | The companion calls `QueueDeliveryRpc.deliver()` with a shared secret and receives typed delivery outcomes. External Node clients continue using HTTP. | Adopted in the internal RPC spike. Delivery logic is extracted from the router, and the old HTTP delivery route is removed. Deploy matching worker scripts together.                                             |
